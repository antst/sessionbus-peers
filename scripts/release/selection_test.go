...
func TestBootstrapReleaseSelection(t *testing.T) {
	for _, role := range []string{"claude", "codex", "grok", "qwen", "opencode", "kilo", "pi", "omp"} {
		for _, tc := range []struct{ name, version, mirror, base string }{
			{"default", "", "", "https://github.com/antst/sessionbus-peers/releases/latest/download"},
			{"latest", "latest", "", "https://github.com/antst/sessionbus-peers/releases/latest/download"},
			...
			t.Run(role+"/"+tc.name, func(t *testing.T) {
				...
				cmd.Env = append(cmd.Env, "PATH="+dir+string(os.PathListSeparator)+os.Getenv("PATH"), "INSTALL_TEST_CURL_ARGS="+log)
				if tc.version != "" {
