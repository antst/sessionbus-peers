...
if [ -n "${SESSIONBUS_DOWNLOAD_ROOT:-}" ]; then
 ...
 release=$(curl -IsS --retry 2 --connect-timeout 10 --max-time 30 -o /dev/null -w '%{http_code} %{redirect_url}' https://github.com/sessionbus/codex-peer/releases/latest)
 case "$release" in
  30[12378]' https://github.com/sessionbus/codex-peer/releases/tag/'*)
   tag=${release#*https://github.com/sessionbus/codex-peer/releases/tag/}
   awk -v tag="$tag" 'BEGIN { exit(tag !~ /^v[0-9]+[.][0-9]+[.][0-9]+$/) }' || { echo "Invalid stable peer release redirect ($release)" >&2; exit 1; }
   base=https://github.com/sessionbus/codex-peer/releases/download/$tag;;
  30[12378]' https://github.com/sessionbus/codex-peer/releases')
   echo 'No stable peer release yet; installing the published development build.' >&2
   base=https://github.com/sessionbus/codex-peer/releases/download/development;;
