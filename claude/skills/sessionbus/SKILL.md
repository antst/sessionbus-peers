...
A Claude lane owns one native session. Its headless carrier has no source-proven
atomic active append boundary, so lane delivery is deferred before any native
write. The daemon starts an idle message immediately or retains it in bounded
memory for the next managed run when a turn is active. A
`queued_for_next_turn` receipt is that scheduling promise; it is not native
admission, durability, or model consumption. Collect the separate terminal
through the completion pointer or oldest unacknowledged record. Native default
permissions may refuse tools that need approval.
