...
test('packed npm install resolves one bin and complete native plugin from a root with spaces', async t => {
  ...
  npm(['install','--global','--prefix',prefix,join(dir,pack.filename),'--offline','--ignore-scripts','--no-audit','--no-fund']);
  const bin=join(prefix,'bin/claude-peer'); const installed=dirname(realpathSync(bin));
  assert.equal(installed,join(prefix,'lib/node_modules/@sessionbus/claude'));
  ...
  assert.equal(existsSync(bin),false);assert.equal(existsSync(installed),false);
});
