...
func TestPublicActionsUseActualConnectionAndSchema(t *testing.T) {
	cases := []struct{ action, args, method, result string }{
		...
		{"wait", `{"session_id":"lane","run_id":"g/1"}`, "turn.wait", `{"session_id":"lane","run_id":"g/1","state":"done","result":{"outcome":"completed","result":"done"}}`},
		{"ack", `{"session_id":"lane","run_id":"g/1"}`, "turn.ack", `{}`},
