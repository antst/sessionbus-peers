...
ack: {session_id:string, run_id:string}; acknowledge the oldest terminal only after receiving and using its result. Repeating that acknowledgment is idempotent, without returning the answer again. No skipping older output. Read errors/unavailable are not successful native terminals. Closing, auto-close or worker loss makes unacknowledged output unavailable; collection does not reset the retirement deadline.
