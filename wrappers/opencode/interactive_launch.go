...
const InteractiveLaunchEnv = "SESSIONBUS_OPENCODE_LAUNCH"
...
// RunInteractive retains only direct-child/transient-directory ownership. All
...
func RunInteractive(ctx context.Context, plan host.ExecPlan) (err error) {
	...
	socket := interactiveEnv(plan.Env, host.SocketEnv)
	if !filepath.IsAbs(socket) {
		return errors.New("managed Sessionbus socket must be absolute")
	}
	if interactiveEnv(plan.Env, host.LocalKeyEnv) != "" {
	...
	var groups []string
	if err = json.Unmarshal([]byte(interactiveEnv(plan.Env, host.GroupsEnv)), &groups); err != nil {
		return err
	}
	binding, err := json.Marshal(interactiveLaunchBinding{Directory: directory, PID: os.Getpid(), Socket: socket, Name: interactiveEnv(plan.Env, host.NameEnv), Groups: groups})
	if err != nil {
		return err
	}
	...
	environment = append(environment, InteractiveLaunchEnv+"="+string(binding))
	if interactiveEnv(environment, "OPENCODE_SERVER_PASSWORD") == "" {
		var secret [32]byte
		if _, err = rand.Read(secret[:]); err != nil {
			return err
		}
		environment = setInteractiveEnv(environment, "OPENCODE_SERVER_PASSWORD", hex.EncodeToString(secret[:]))
	}
