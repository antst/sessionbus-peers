...
export async function interactiveActivation(environment, parent = process.ppid) {
  const raw = environment.SESSIONBUS_OPENCODE_LAUNCH;
  if (!raw) return null;
  if (typeof raw !== "string" || Buffer.byteLength(raw) > 64*1024) throw new Error("invalid managed OpenCode launch metadata");
  const launch = JSON.parse(raw);
  if (!launch || typeof launch !== "object" || Array.isArray(launch) || Object.keys(launch).length !== 5 || !Number.isSafeInteger(launch.pid) || launch.pid <= 1 || typeof launch.directory !== "string" || !path.isAbsolute(launch.directory) || typeof launch.socket !== "string" || !path.isAbsolute(launch.socket) || typeof launch.name !== "string" || !Array.isArray(launch.groups) || launch.groups.some((value) => typeof value !== "string" || !value.length) || new Set(launch.groups).size !== launch.groups.length) throw new Error("invalid managed OpenCode launch metadata");
  ...
  if (!directory.isDirectory() || (directory.mode & 0o777) !== 0o700 || directory.uid !== process.getuid()) throw new Error("managed OpenCode launch directory is not private and owned");
