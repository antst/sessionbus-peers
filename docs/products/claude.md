...
Claude Code 2.1.276 (binary SHA-256 `8a56c8a14bd3cb246e2bdb7e60aefe0f609bff78c8bbcc5ea6b1817c111c6145`) defines both `--disallowedTools` spellings as a variadic comma-or-space-separated deny list. Its retained option scanner consumes one required value plus later non-option values for the separate form, treats an attached `=` value as that occurrence alone, and stops at native `--`. Its `dp` rule splitter separates comma or literal-space tokens only outside parentheses, then compares rules after JavaScript `trim` (binary byte offsets 192059656 and 190791441). The guard follows ECMAScript [TrimString](https://tc39.es/ecma262/multipage/text-processing.html#sec-trimstring) and [White Space](https://tc39.es/ecma262/multipage/ecmascript-language-lexical-grammar.html#sec-white-space), including U+FEFF and excluding U+0085. This 2.1.276 source reading is separate from the sealed UMKA 2.1.271 version preflight and is not an upper version pin.

The retained 0.4.0 launcher preserved native Claude arguments around wrapper-owned fields, while its product catalog unconditionally supplied the then-current managed MCP allow rule and kept the native bypass launch policy. Sources: `/home/antst/claude-architecture-20260908/sources/v0.4.0/internal/launcher/claude_peer.go:15-81` and `/home/antst/claude-architecture-20260908/sources/v0.4.0/internal/productcatalog/catalog.go:163-170`. The current exact-tool guard restores the owner's required communications invariant without widening that earlier argument ownership.
